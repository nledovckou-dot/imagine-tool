...
def gpt_creative_prompts(idea: str, ref_images: list[tuple[bytes, str]] | None = None) -> tuple[str, str]:
    system_msg = (
        ...
        "  Если уместно — текст/слоган. На РУССКОМ. Макс 600 символов.\n\n"
        "ПРОМПТ 2 — ВИДЕО (для Sora, 8 сек рекламный ролик):\n"
        "  ДИНАМИЧНЫЙ имиджевый ролик, НЕ статика!\n"
        "  — Движение камеры (наезд, облёт, панорама, слоу-мо, дрон)\n"
        "  — Активность (люди, природа, частицы, блики, дым, вода)\n"
        "  — Смена планов (крупный → средний → общий)\n"
        "  — Кинематографичный свет и атмосфера\n"
        "  — Текст/слоган появляется и исчезает\n"
        "  На РУССКОМ. Макс 800 символов.\n\n"
